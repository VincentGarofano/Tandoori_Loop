function Calcul_Te_TRG_ViaInterface(Overwrite,InfoGraphes,InfoTe,InfoNe,InfoHMDSO,filetype,Tg,P_tot,longueur,FctRep,ChoixErreur,ChoixHautePression,ChoixAutoabs,Commentaire,flow,exposant,Dimension,sig_longueur)
...
% Range de densité électronique scannée (m^-3)
   ...
   % Range de densité électronique scannée (s^1)
   HMDSO =logspace(log10(InfoHMDSO(1)/InfoHMDSO(2)),log10(InfoHMDSO(1)*InfoHMDSO(2)),InfoHMDSO(3));
...
NeOpt           =zeros(length(fileNames),length(HMDSO));
NeMin           =zeros(length(fileNames),length(HMDSO));
NeMax           =zeros(length(fileNames),length(HMDSO));
TeOptimal       =zeros(length(fileNames),length(HMDSO));
TeMin           =zeros(length(fileNames),length(HMDSO));
TeMax           =zeros(length(fileNames),length(HMDSO));
MoyenneOptimale =zeros(length(fileNames),length(HMDSO));
ErreurOptimal   =zeros(length(fileNames),length(HMDSO));
...
disp ('Calcul de l''intensité théorique des raies...')
 ...
 %% sélection des taux de réactions
    ...
    wait=0;
    global gaz_i gaz_f nu_hmdso
for ite_HMDSO=1:length(HMDSO)
nu_hmdso = HMDSO(ite_HMDSO);
for gaz=gaz_i:gaz_f %On fait le calcul théorique de l'intensité de raies pour 2=néon, 3=argon, 4=krypton et 5=xénon
    ...
    %pour mettre les valeurs dans un vecteur avec tous les gaz inclus
       ...
       waitbar(wait/(length(gaz_i:gaz_f)*length(Ne)*length(HMDSO)),h,'Calcul théorique: sélection des Taux de réactions') 
    ...
    for j=1:length(Ne)
        waitbar(wait/(length(gaz_i:gaz_f)*length(Ne)*length(HMDSO)),h,'Calcul théorique') 
        wait=wait+1;
...
if ite_HMDSO ==1
    if exist('Te.txt', 'file')==0 % n'écrit pas le Header si il est deja fait
        ...
        formatSpec = '%-22s\t %s \t%s \t%s \t%s \t%s \t%s \t%s \t%s \t%s \t%s \t%s \t%s \t%s\r\n';  
        fprintf(fileID,formatSpec,'------NomFichier------','TeOpt','TeMin','TeMax','NeOpt   ','nuOpt   ','nuMin','nuMax','% STD','Poids','exp','HighP','AutoAbs','Commentaire');
    else
        formatSpec = '%-22s\t %s \t%s \t%s \t%s \t%s \t%s \t%s \t%s \t%s \t%s \t%s \t%s \t%s\r\n';  
        fileID = fopen('Te.out','a');   
    end
    ...
    if exist('Nm_Argon.out', 'file')==0 % n'écrit pas le Header si il est deja fait
       ...
       % fprintf(fileID3,formatSpec3,'1s2','1s3','1s4','1s5');
        ...
        if exist('nununu.out', 'file')==0 % n'écrit pas le Header si il est deja fait
        fileID4 = fopen( 'nununu.out','a');         
        formatSpec4 = '%s \t%s \t%s \t%s\r\n';
        fprintf(fileID4,formatSpec4,'nu','%STD','Ne','Te');
    else
        fileID4 = fopen('nununu.out','a');   
        formatSpec4 = '%s \t%s \t%s \t%s\r\n';
    end
...
for k=1:length(fileNames)
    ...
    if nbrderaies(k) > 1             %Vérification s'il y a au moins 2 raies à ajuster      
            %% Calcul de l'erreur
            [STDErr,NeOptimal,TempOptimal,mecanismeOptimal(k),MoyenneOptimale(k),ErreurOptimal(k,ite_HMDSO),FitCorrige(k,:)] = TeCalculErreur_TRG(ChoixErreur,Ne,Te,mecanismes,I_exp(k,:),I_theo,FitCorrige(k,:),P,sig_I_theo,sig_I_exp(k,:));

            %% Extraction des résultats
            NeOpt(k,ite_HMDSO)=NeOptimal(1); NeMin(k,ite_HMDSO)=NeOptimal(2); NeMax(k,ite_HMDSO)=NeOptimal(3); PosNeMin(1)=NeOptimal(4);
            TeOptimal(k,ite_HMDSO)=TempOptimal(1); TeMin(k,ite_HMDSO)=TempOptimal(2); TeMax(k,ite_HMDSO)=TempOptimal(3); PosTeMin(1)=TempOptimal(4);
...
%              x(:) = densite1s(3,PosNeMin(1),PosTeMin(1),:)
             ...
             %  fprintf(fileID3,formatSpec3,densite1s(3,PosNeMin(1),PosTeMin(1),2:5));
               Metas_Optimal(k,ite_HMDSO,:) = densite1s(3,PosNeMin(1),PosTeMin(1),:);
...
[val_HMDSO pos_HMDSO] = min(ErreurOptimal(k,:));
ite_max(k) =pos_HMDSO;
HMDSOopt(k) = HMDSO(pos_HMDSO) ;

% prend le min et le Max de HMDSO
for m=1:length(ErreurOptimal(k,:))
  if ErreurOptimal(k,m) < 1.02*min(ErreurOptimal(k,:)) % 2%
      HMDSO_min_max(k,m)  =  HMDSO(m);
  else
      HMDSO_min_max(k,m)  = HMDSOopt(k);
  end
end
HMDSOMin(k)  = min(HMDSO_min_max(k,:));
HMDSOMax(k)  = max(HMDSO_min_max(k,:));

    if TeGraph1==1 
      figure2=figure;
        set(figure2,'numbertitle','off')%,'Position',[200 600 1000 800])
    subplot(3,1,1)
    semilogx(HMDSO,ErreurOptimal(k,:),'LineWidth',1.5)
    ylabel('% STD','FontSize',11,'fontweight','bold');
     subplot(3,1,2)
    semilogx(HMDSO,NeOpt(k,:),'linewidth',1.5)
    ylabel('Ne (m-3)','FontSize',11,'fontweight','bold');
      subplot(3,1,3)
      semilogx(HMDSO,TeOptimal(k,:),'linewidth',1.5)
      xlabel('quenching nu s-1','FontSize',11,'fontweight','bold'); ylabel('Te (eV)','FontSize',11,'fontweight','bold');

    end
    for m=1:length(HMDSO)
       Table4{1}=num2str(HMDSO(m),4);
       Table4{2}=num2str(ErreurOptimal(k,m),4);
       Table4{3}=num2str(NeOpt(k,m),4);
       Table4{4}=num2str(TeOptimal(k,m),5);
       fprintf(fileID4,formatSpec4,Table4{:});
    end       
...
for k=1:length(fileNames)
            %% Écriture des paramètres optimaux dans un fichier
            formatSpec = '%-22s\t %-5s \t%-5s \t%-5s \t%-10s \t%-10s \t%-10s \t%-10s \t%-6s \t%-5s \t%-5s \t%-5s \t%-6s \t%-20s\r\n';  
            Table1{1}=char(fileNames(k));
            Table1{2}=num2str(TeOptimal(k,ite_max(k)),4);
            Table1{3}=num2str(TeMin(k,ite_max(k)),4);
            Table1{4}=num2str(TeMax(k,ite_max(k)),4);
            Table1{5}=num2str(NeOpt(k,ite_max(k)),4);
            Table1{6}=num2str(HMDSOopt(k),4);
            Table1{7}=num2str(HMDSOMin(k),4);
            Table1{8}=num2str(HMDSOMax(k),4);
            Table1{9}=num2str(ErreurOptimal(k,ite_max(k)),4);
            Table1{10}=num2str(ChoixErreur);
            Table1{11}=num2str(exposant);
            Table1{12}=num2str(ChoixHautePression);
            Table1{13}=num2str(ChoixAutoabs);
            Table1{14}=Commentaire;
            fprintf(fileID,formatSpec,Table1{:});
            
               fprintf(fileID3,formatSpec3,Metas_Optimal(k,ite_max(k),2:5));
